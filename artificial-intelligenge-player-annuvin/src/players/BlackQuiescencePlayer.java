...
/**
 * Implementazione del giocatore NERO che utilizza il MIN-MAX.
 */
public class BlackQuiescencePlayer implements Player {
	 ...
	 */
	public BlackQuiescencePlayer(Heuristic heuristic, int maxConfigurationNumber){
	...
	public final BoardConfiguration getBestNextConfiguration(BoardConfiguration c){
		...
		if(currentMax >= 10){
			...
			byte bestDifference = (byte)(currentbestConfig.getNumberOfBlacks() - currentbestConfig.getNumberOfWhites());
			for(BoardConfiguration config: sameValue){
				byte currentDifference = (byte)(config.getNumberOfBlacks() - config.getNumberOfWhites());
	...
	private final void labelTree(BoardConfiguration[] treeExpansion){
		for(int k=treeExpansion.length-1; k>=0; k--){
			...
			if(sons.isEmpty()){
				bc.setValue((byte)heuristic.evaluate(bc));
			}
			else if(bc.getTurn()==GlobalValues.BLACK_TURN)
