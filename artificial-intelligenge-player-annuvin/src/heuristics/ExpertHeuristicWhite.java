...
public class ExpertHeuristicWhite implements Heuristic{
	...
	public final byte evaluate(BoardConfiguration bc) {
		...
		byte rule1 = evaluateDistanceInWhites(bc);
		byte rule2 = evaluateWhiteBlackDifference(bc);
//		byte rule3 = evalutateDistanceInBlacks(bc);
//		byte rule4;
...
//		
	...
	private final byte evaluateDistanceInWhites(BoardConfiguration bc){
		byte meanValue = 0;
		...
		for(byte pi=0; pi<37; pi++){
			for(byte pj=(byte)(pi+1); pj<37; pj++){
				if(distancesInWhites[pi][pj] != 10){
					summary += Math.abs(averageDistance-distancesInWhites[pi][pj]);
				}
			}
		}
	...
	private final byte evaluateWhiteBlackDifference(BoardConfiguration bc){
		byte numberOfWhite = bc.getNumberOfWhites();
		byte numberOfBlack = bc.getNumberOfBlacks();
		byte difference = (byte)(numberOfWhite-numberOfBlack);
	...
	@Deprecated
	private final byte evalutateDistanceInBlacks(BoardConfiguration bc){
	...
	private final byte evaluateOffensiveReachability(BoardConfiguration bc){
		 ...
		 * si scorrono le bianche e se si trova una bianche che la raggiunge,
